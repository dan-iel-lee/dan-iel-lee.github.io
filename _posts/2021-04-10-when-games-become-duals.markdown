...
They say its unhealthy to see things as zero-sum... They are wrong (sometimes). In fact, zero-sum games can be incredibly useful. While it's hard to say/prove things about games (even restricted to two players) in general (with Nash's theorem as a standout exception), adding the constraint that players are strictly adversarial gives us enough structure to prove the minimax theorem—arguably the most important theorem in game theory. Yet despite the extra structure, they're still widely applicable. They're great for modeling excludable resources, and have been used to great success in the theory of [Generative Adversarial Nets](https://arxiv.org/abs/1406.2661) (GANs) and [online learning](https://toc4fairness.org/how-to-estimate-the-uncertainty-of-predictions/). 
